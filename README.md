...
Selections persist across pi restarts via `pi.appendEntry`. The browser UI is served from a localhost-only HTTP+WebSocket server with a per-invocation auth token (same pattern as `pi-studio`).
